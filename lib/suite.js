...
let InvalidSpecificationError = customError('InvalidSpecificationError');
...
export default class Suite extends EventEmitter {
    ...
    async launch() {
        ...
        try {
            ...
            let error = new LoadingSpecDependencySuiteError(`${err.name} error occurred while loading the dependencies \n ${err.message}`);
            this.status = 'fail';
    ...
    async loadSpecDependencies() {
        if (this.data.spec_dependencies) {
            for (let dependency of this.data.spec_dependencies) {
                let file = path.resolve(process.cwd(), dependency);
        ...
        if (suiteConfigData.custom_configuration) {
            ...
            try {
                if (suiteConfigData.custom_configuration.run_type === 'inline') {
                    ...
                    const module = assertFileValidity(suiteConfigData.custom_configuration.module.module_path, 'Custom Configuration module');
        ...
        try {
            if (loopData.type === 'static') {
                ...
                if (loopData.dynamic.run_type === 'inline') {
                    ...
                    const module = assertFileValidity(loopData.dynamic.module.module_path, 'Loop data module');
                    const customModule = loadModule(module);
    ...
    async runHook(type, hookData) {
        ...
        if (data.run_type === 'inline') {
            ...
            if (data.module) {
                const module = assertFileValidity(data.module.module_path, 'Suite ${type} hook module');
                const customModule = loadModule(module);
